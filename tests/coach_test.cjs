...
ok(dna.indexOf(dsemHoje)>=0,'relógio com o DIA DA SEMANA ('+dsemHoje+')');
ok(/agora são \d{1,2}:\d{2}/.test(dna),'relógio com a HORA certa');
ok(dna.indexOf('NUNCA diga que não sabe')>=0,'proibido confessar ignorância de data');
ok(dna.indexOf('fuso dele')>=0,'relógio menciona o fuso da pessoa');
